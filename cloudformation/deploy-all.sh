...
# Assume the CloudFormation deployment role
...
deploy_stack "gits-ecr" "cloudformation/ecr.yaml" "" "ProjectName=$PROJECT_NAME ImageTagMutability=MUTABLE ScanOnPush=true EncryptionType=AES256"
...
deploy_stack "gits-s3" "cloudformation/s3.yaml" "" "BucketName=${PROJECT_NAME}-artifacts EnableVersioning=true BlockPublicAccess=true RetainOnDelete=false"

deploy_stack "gits-dynamodb" "cloudformation/dynamodb.yaml" "" "TableName=${PROJECT_NAME}-jobs PointInTimeRecovery=ENABLED BillingMode=PAY_PER_REQUEST"

deploy_stack "gits-secret-manager" "cloudformation/secretmanager.yaml" "" "ProjectName=$PROJECT_NAME GitHubToken=$GITHUB_TOKEN"

deploy_stack "gits-codebuild" "cloudformation/codebuild.yaml" "" "ProjectName=$PROJECT_NAME ArtifactBucketName=${PROJECT_NAME}-artifacts"

deploy_stack "gits-lambdas" "cloudformation/lambdas.yaml" "--capabilities CAPABILITY_IAM" "ProjectName=$PROJECT_NAME DynamoTableName=${PROJECT_NAME}-jobs ArtifactBucketName=${PROJECT_NAME}-artifacts CodeBuildProjectName=$PROJECT_NAME ImageUriSchedule=$IMAGE_URI_SCHEDULE ImageUriDelete=$IMAGE_URI_DELETE ImageUriStatus=$IMAGE_URI_STATUS ImageUriCodeBuildLens=$IMAGE_URI_CODEBUILD_LENS"

deploy_stack "gits-events" "cloudformation/eventbridge.yaml" "" "ProjectName=$PROJECT_NAME"

deploy_stack "gits-apigateway" "cloudformation/apigateway.yaml" "" "ProjectName=$PROJECT_NAME"
