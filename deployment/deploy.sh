...
while [[ $# -gt 0 ]]; do
	case "$1" in
		...
		--getstatus-only)
			...
			;;
		-h|--help)
			echo "Usage: $0 [--publish] [--backend-only | --lambda-only | --codebuildlense-only | --getstatus-only]"
			exit 0
			;;
		*)
			echo "Unknown argument: $1" >&2
			echo "Usage: $0 [--publish] [--backend-only | --lambda-only | --codebuildlense-only | --getstatus-only]" >&2
			exit 2
			;;
	esac
	shift || true
...
chmod +x "$BACKEND_SCRIPT" "$LAMBDA_SCRIPT" "$CODEBUILDLENSE_SCRIPT" "$GETSTATUS_SCRIPT"
